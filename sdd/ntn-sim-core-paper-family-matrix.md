...
| `FAM-RT-ACCESS-VALID` | `real-trace-validation` + access family | TLE-backed access / HO validation papers | real-trace OMM/TLE ingest + SGP4-sampled cache | inherited from `FAM-ACCESS-SYNTH` | orbit realism, timing realism, replay curation, cross-mode parity | validates access-family timing realism; does not define new beam/channel family | `beamHO-bench`, `ntn-stack`, `leo-simulator` | broader scalability / mixed-orbit promotion remains separately blocked |
| `FAM-RT-MB-VALID` | `real-trace-validation` + multibeam or BH family | real-trace multibeam / BH validation studies | real-trace OMM/TLE ingest + SGP4-sampled cache | inherited from `FAM-MB-HOBS-SYNTH` or `FAM-BH-SYNTH` | cross-mode parity under real constellation timing | validates whether multibeam/BH conclusions survive real-trace orbit timing | `ntn-stack`, `leo-simulator`, `beamHO-bench` | broader scalability / mixed-orbit promotion remains separately blocked |
...
| `FAM-RT-ACCESS-VALID` | allowed | allowed for parity studies | allowed for validation-sized parity after T1 truth-path correction | blocked for larger-catalog / mixed-orbit follow-on until separately promoted |
| `FAM-RT-MB-VALID` | allowed | allowed for exploratory checks | allowed for validation-sized parity after T1 truth-path correction | blocked for broader multibeam/BH real-trace follow-on until separately promoted |
