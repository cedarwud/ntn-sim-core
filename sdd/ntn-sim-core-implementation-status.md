...
| real-trace truth-path correction | ▶ T1 active / ready to start | Active authority is `sdd/real-trace-truth-path-correction-outline.md`; scope is limited to SGP4-sampled real-trace cache truth, surviving wording/validator sync, and the planned `VAL-RT-003` / `VAL-RT-004` gates |
| real-trace scalability follow-on | ⏸ blocked (`no-go` preflight on 2026-04-01) | Future mixed-orbit `OMNeT++` work may still require larger-catalog planning, but this line stays paused until T1 truth-path correction is closed or explicitly declined; keep `sdd/real-trace-scalability-preflight-note.md` as the blocked decision record |
