...
| paper-mode / claim-mode hardening (`PM1`) | 🔄 active follow-on (promoted 2026-04-02) | Active authority is `sdd/paper-mode-claim-mode-hardening-outline.md`; this line freezes `PAP-2024-MORL-MULTIBEAM` / `modqn-paper-baseline` as the current anchor baseline, defines main-result vs robustness vs sensitivity paper usage, constrains assumption-heavy `EE / power` claims, and requires future paper targets to land as sibling baselines rather than replacing the anchor |
...
| `sdd/ntn-sim-core-research-positioning-note.md` | current paper-oriented project positioning, corpus-relative gap ranking, and future-direction guidance | active |
| `sdd/paper-mode-claim-mode-hardening-outline.md` | active paper-oriented governance follow-on for main-result / robustness / sensitivity discipline and sibling-baseline extension rules | active |
