...
**Version:** 4.8.22
**Date:** 2026-04-15
**Status:** Prior hardening/closure program complete; targeted reruns of `validate:visual-browser`, `validate:profiles`, `validate:contracts`, `validate:bundle`, and `validate:runtime` pass after the truth-preserving showcase realignment landing on 2026-04-15, though browser-sensitive evidence should still be treated as transient-sensitive rather than permanently de-flaked. That landing promotes `sdd/truth-preserving-showcase-visual-realignment-follow-on.md` into active narrow authority: `SceneShell` now builds a shared `BeamPresentationFrame` and hands it to `SatelliteSkyLayer`, `EarthMovingBeamLayer`, `EarthFixedCellLayer`, `BeamInfoOverlay`, and `HandoverLinkOverlay`, so display sats / event sats / beam sats / primary beams / context beams / marker roles are no longer independently re-derived by each renderer. The same slice strengthens serving/prepared/post-HO/neutral visual separation without mutating engine SINR, handover, or bounded-steering truth, removes lingering UE re-centering from the BH-cell analysis helper, and now keeps idle-pass satellites on a serving-centered local active neighborhood instead of a 1-2 beam collapse while suppressing continuity-focused wide-angle scatter by falling back to a serving-centered beam narrative when no nearby supporting satellite exists in truth. `EarthFixedCellLayer` consumes the same frame-level beam picks as the moving-beam renderer, the dedicated `case9-daps-showcase` profile (`1 UE`, BH-disabled, curated epoch `2026-01-01T00:45:00Z`, showcase-only `hysteresis_db = 0`, `pingPongWindowSec = 15`) remains the interactive default, and browser validation now covers central-readability / scatter-suppression / truth-preservation (`VAL-FV-005` / `VAL-FV-006` / `VAL-FV-010`) against raw snapshot beam truth published by the validation probe rather than renderer-derived geometry samples. Simulator Platform Refactor is complete through Phase 5 Group 3 (2026-03-31), and downstream architecture is now complete through Group 2 (2026-03-31; authority/docs synchronized 2026-04-01). Final platform audit re-check passed on 2026-03-31 after stabilizing the DAPS live browser gate: `useSimulation.ts` now preserves missed discrete ticks and holds short-lived dual-active snapshots long enough for browser probes to observe them, while `engine/tick.ts` advances handover/KPI/energy exactly once per discrete tick. A follow-on continuity hardening slice landed on 2026-04-11: the DAPS manager now consumes the authored TTT / EMA / ping-pong / RLF envelope instead of dropping back to a timer-only trigger path, snapshot truth now exposes prepared and dual-active continuity roles from manager state (including replay playback), `ControlPanel` adds an `HO Slow` visual-only toggle that auto-clamps playback to `1x` while prepared or dual-active handover truth is visible, replay curation for DAPS / MC-HO now uses continuity-aware window selection instead of pass-only geometry heuristics, and the benchmark-facing `case9-daps-baseline` now sits alongside the dedicated `case9-daps-showcase` first-screen default rather than carrying both jobs at once. The DAPS family still uses the same 2026-04-11 tuning on the benchmark-facing baseline: preparation begins once the source serving elevation drops below `30°` if a stronger real-time SINR candidate exists, the hard release floor stays at `10°`, and the dual-active ceiling remains `3s` so the 1 s discrete runtime can complete a real path switch instead of repeatedly falling back to the source; the showcase sibling now keeps those truth-path rules but uses a shorter ping-pong guard than the benchmark baseline while still suppressing the visible post-switch return-to-source oscillation that the earlier 5 s showcase tuning allowed. Phase 1 closure is hardened in the current tree: `validate-parameter-registry.mjs` now machine-enforces profile-specific registry/runtime parity in addition to coverage/source/namespace checks. Phase 4 (Runtime Contract Freeze) remains the frozen consumer-boundary baseline: `src/core/contracts/`, `RunnerExposureApi`, and `getProfileList()` semantics were preserved while Phase 5 reworked internal ownership. Phase 5 Group 1 froze the split/retirement plan and reviewer-grade `VAL-PLAT-011/012` contract; Group 2 landed `src/core/engine/`, split `profiles/types.ts`, moved orbit bootstrap ownership into `src/core/orbit/profile-runtime.ts`, and scoped the `parameter-registry` size split; Group 3 retired browser sync XHR real-trace bootstrap in both `useSimulation.ts` and `useReplay.ts`, resolved the beam-selection naming collision, deleted `profile-composer.ts`, retired runtime `ProfileConfig.sourceMap`, moved authored exposure/provenance/materialization responsibilities into `profile-exposure-catalog.ts`, `profile-authoring-registry.ts`, `runtime-materialization.ts`, and `profile-provenance-view.ts`, and machine-enforced `VAL-PLAT-011/012` in `validate-structure.mjs`. Downstream MODQN M3 is now complete: the shipped M2 result surface was stabilized additively, `ModqnViewModel` landed as the truth-safe UI projector, `validate-modqn-m3.ts` now runs the real reproduction path instead of a mock, and `validate:modqn:m2` / `validate:modqn:m3` / `validate:stage` all pass on 2026-04-01. A same-day T1 real-trace truth-path correction also landed: validation-sized real-trace cache samples now preserve SatRec-backed SGP4 truth during cache construction, `VAL-RT-003` / `VAL-RT-004` pass, and the shipped path is truthfully described as OMM/TLE ingest plus an SGP4-sampled cache-backed runtime/replay path. Phase 03A MODQN bundle replay is now complete through the consumer-side path: Slice B shipped the typed adapter seam on 2026-04-13, and the 2026-04-14 consumer follow-on added explicit `native-live` / `native-replay` / `modqn-bundle` truth-source switching in `SceneShell`, a bundle-backed replay hook/view-model path, slot stepping controls, shared beam/link presentation over bundle truth, and a dedicated assumptions / provenance / training-eval disclosure panel that explicitly rejects native-default wording. Validation now includes both `validate:modqn:bundle` (`VAL-MODQN-BUNDLE-001`) and `validate:modqn:bundle-ui` (`VAL-MODQN-BUNDLE-002`) in the standard `validate:stage` chain. MODQN baseline reproduction is therefore complete through M3 Group 1 plus the Phase 03A consumer replay handoff, while `estnet` remains paused. The broader real-trace scalability line stays blocked/paused for any mixed-orbit or larger-catalog follow-on until it is separately re-promoted. PM1 paper-mode / claim-mode hardening is now complete as a shipped governance prerequisite. TP1 is now also complete as a shipped paper-evidence surface: `runModqnAnchorParityBundle()` / `validate-modqn-parity.ts` package the shipped `PAP-2024-MORL-MULTIBEAM` / `modqn-paper-baseline` anchor into a range-faithful envelope target, while the current user-count / satellite-count / user-speed sweeps and comparator ranking remain qualitative-only under the disclosed proxy ceiling; this avoids reopening runtime architecture. EP1 `EE / power` realism hardening is now also landed as a narrow shipped paper-safety surface: active-TX EE vs broader communication-power proxy semantics are split explicitly, run artifacts expose `eePowerDisclosure`, assumption-backed denominator terms no longer hide behind runtime naming drift, and `VAL-EE-003` / `VAL-EE-004` pass in the current tree. This remains intentionally narrower than backend/protocol realism, orbit scalability, or new algorithm work.
...
| VAL-MODQN-BUNDLE-001 | Phase 03A Slice B | ✅ pass | `scripts/validate-modqn-bundle-adapter.ts` — typed consumer adapter rejects schema/version/geometry drift, loads the producer fixture bundle, and preserves replay-frame grouping without recomputing truth |
| VAL-MODQN-BUNDLE-002 | Phase 03A Slice C/D | ✅ pass | `scripts/validate-modqn-bundle-ui.ts` — browser automation proves `mode=modqn-bundle` swaps the active truth source, slot stepping advances the exported bundle timeline, shared beam/link presenters consume bundle truth, and bundle assumptions/provenance/training disclosure stays separate from native defaults |
...
**Note:** Formula-level (`-F`) tests pass. Engine-level (`-E`) golden cases also pass, and `npm run validate:stage` succeeds using `node --import tsx` for the golden-engine step. Browser-level (`-V`) closure evidence is automated for the current explainability/continuity package; screenshot packs remain supplementary evidence. EP1 follow-on gates `VAL-EE-003` / `VAL-EE-004` plus the Phase 03A bundle-consumer gates `VAL-MODQN-BUNDLE-001` / `VAL-MODQN-BUNDLE-002` are now part of the standard `validate:stage` chain.
