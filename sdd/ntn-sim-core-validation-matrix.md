...
| `VAL-MODQN-BUNDLE-005` | downstream MODQN producer diagnostics consumer surface | reserved for future producer-owned policy diagnostics / explainability over additive bundle fields; should prove diagnostics come from exported producer truth, older bundles disclose absence without invention, and Slice 2/3/4 behavior remains intact | Slice 5 draft / pending |
...
| VAL-MODQN-BUNDLE-005 | V (reserved) | pending future browser automation for producer-owned decision diagnostics / explainability over additive bundle fields; not active until the paired producer-side Phase 03B export slice lands |
...
| VAL-MODQN-BUNDLE-005 | Slice 5 | ⏸ RESERVED | Planned future gate for producer-owned diagnostics / explainability once the paired producer export slice lands; not active in the current tree |

**Note:** Formula-level (`-F`) checks are automated and pass. Engine-level (`-E`) checks pass and are part of `npm run validate:stage` via `node --import tsx`. Engine coverage now extends through E-11, including `VAL-POLICY-001` and `VAL-DOPPLER-001` in addition to the earlier E-5 through E-9 expansion. `npm run validate:integration` runs all engine + reproduction checks. Browser-level visual checks (`VAL-BEAM-001`, `VAL-FV-004` through `VAL-FV-010`, `VAL-EXP-001`, `VAL-MODQN-BUNDLE-002`, `VAL-MODQN-BUNDLE-003`, and `VAL-MODQN-BUNDLE-004`) are automated via `validate-visual-browser.ts` / `validate-modqn-bundle-ui.ts`, and `VAL-ORB-001` is now browser/headless-hybrid validated by `validate-orbit-parity.ts`. Platform Refactor contract gates VAL-PLAT-008/009/010 are enforced via `validate-contracts.mjs`, Phase 5 structural gates VAL-PLAT-011/012 are enforced via `validate-structure.mjs`, EP1 `EE / power` hardening gates VAL-EE-003/004 are enforced via `validate-ee-provenance.ts` and `validate-ee-disclosure.ts`, and the Phase 03A bundle-consumer gates VAL-MODQN-BUNDLE-001/002/003/004 are enforced via `validate-modqn-bundle-adapter.ts` and `validate-modqn-bundle-ui.ts`. `VAL-MODQN-BUNDLE-005` is reserved only; it is not active until the paired producer-side diagnostics export slice lands. The latest targeted reruns passed, but the browser-visible subset should still be treated as transient-sensitive.
