...
| `VAL-MODQN-BUNDLE-002` | downstream MODQN bundle UI | bundle replay mode changes the active truth source, replay-incomplete bundles are rejected before presentation, slot controls advance the exported timeline, shared beam/link renderers consume exported slot truth, native overlay state restores after bundle-mode exit, and assumptions/provenance/training disclosure remains distinct from native defaults | Phase 03A Slice C/D |
...
| VAL-MODQN-BUNDLE-001 | structural + engine | validate-modqn-bundle-adapter.ts (schema/version guard, timeline parser, fixture load, and replay-frame grouping over the producer bundle contract) |
| VAL-MODQN-BUNDLE-002 | V | validate-modqn-bundle-ui.ts (preflight replay-presentation guard plus browser automation for truth-source switching, slot stepping, exported slot truth in validation-probe/shared renderers, native overlay restoration, and assumptions/provenance/training disclosure) |
