...
**Version:** 2.3.19
**Date:** 2026-04-16
**Status:** Active — enforced Formula/Engine gates passing; browser-visible gates pass in the latest targeted reruns after the truth-preserving showcase realignment landing plus the landed MODQN Slice 2 external bundle loading, Slice 3 story-dashboard follow-on, Slice 4 replay-truth hardening / showcase acceptance follow-on, and Slice 5 producer diagnostics consumer follow-on. `validate:visual-browser` continues to enforce the shared `BeamPresentationFrame` scene grammar, the dedicated `case9-daps-showcase` first-screen path, central high-elevation readability, continuity-focused scatter suppression, and presentation-frame truth preservation. Engine coverage remains through E-11; no hardening IDs are deferred. Platform Refactor is complete through Phase 5 Group 3 (2026-03-31). Final platform audit re-check passed on 2026-03-31 after re-hardening `validate:visual-browser`: the DAPS live probe now sees short-lived dual-active truth via sticky transient snapshot publication in `useSimulation.ts`, while `engine/tick.ts` prevents multi-advance of handover/KPI/energy within a single discrete tick. Phase 1 closure remains hardened through `validate-parameter-registry.mjs`, and Phase 4 Group 2 remains hardened through `validate-contracts.mjs`. Phase 5 Group 3 remains structurally closed through `validate-structure.mjs`. As of 2026-04-16, downstream gates `VAL-MODQN-001`, `VAL-MODQN-002`, `VAL-MODQN-003`, `VAL-MODQN-004`, `VAL-MODQN-BUNDLE-001`, `VAL-MODQN-BUNDLE-002`, `VAL-MODQN-BUNDLE-003`, `VAL-MODQN-BUNDLE-004`, and `VAL-MODQN-BUNDLE-005` all pass via `validate-modqn-baseline.ts`, `validate-modqn-m2.ts`, `validate-modqn-m3.ts`, `validate-modqn-parity.ts`, `validate-modqn-bundle-adapter.ts`, and the expanded `validate-modqn-bundle-ui.ts`; `VAL-MODQN-BUNDLE-001` covers the landed `001G` legacy decision-mask compatibility guard, `VAL-MODQN-BUNDLE-002` covers browser-side `external-directory` loading/error/reset semantics, `VAL-MODQN-BUNDLE-003` covers the richer bundle story-dashboard, bundle-backed KPI/chart surfaces, and disclosure separation, `VAL-MODQN-BUNDLE-004` covers dashboard/HUD/probe truth alignment, authoritative replay wording, shared beam/link replay-truth proof, and a non-trivial external bundle variant through the same scene/dashboard path, and `VAL-MODQN-BUNDLE-005` now covers producer-owned diagnostics rendering, older-bundle absence disclosure, manifest metadata/disclosure separation, sample/external parity, and multi-user slot anchoring to the first exported replay row. `validate:stage` now includes both the Phase 03A adapter gate and the shared bundle truth-source UI gate, carrying `001` through `005`. T1 real-trace truth-path correction remains landed and enforced through `validate-real-trace-truth-path.ts` (`VAL-RT-003`) and `validate-real-trace-wording.mjs` (`VAL-RT-004`). EP1 `EE / power` hardening gates also remain passing through `validate-ee-provenance.ts` (`VAL-EE-003`) and `validate-ee-disclosure.ts` (`VAL-EE-004`).
...
| `VAL-BEAM-001` | beam geometry | earth-moving beam footprint projection matches the declared tracked-lattice access-profile geometry contract | 2 |
...
| `VAL-FV-006` | visualization | beam/SINR overlay and `BeamPresentationFrame` remain derived from snapshot/trace truth and do not recompute authoritative SINR/HO decisions locally | 3 |
...
| `VAL-FV-005` | visualization | beam display membership and scatter suppression remain consistent with shared presentation-frame scene grammar plus observer-sky pass semantics | 3 |
...
| `VAL-RT-003` | real-trace truth-path | validation-sized real-trace cache samples match `propagateSgp4()` / satellite.js at sampled ticks without changing replay identity semantics | T1 |
| `VAL-RT-004` | real-trace truth-path | defaults, registry notes, UI exposure wording, implementation status, and validator assumptions all describe the same shipped real-trace runtime truth without overstating per-tick SGP4 | T1 |
...
| `VAL-EE-003` | energy realism hardening | formulas, parameter spec, provenance inventory, runtime names, and authored defaults agree on active-TX EE vs broader communication-power proxy semantics, and assumption-backed Layer-1 power terms are not surfaced as `Realistic` defaults | EP1 |
| `VAL-EE-004` | energy realism hardening | denominator-sensitive EE artifact paths carry explicit assumption/disclosure surfaces, and HO-energy / energy-centered claim paths stay sensitivity-qualified unless a stronger package is declared | EP1 |
...
| `VAL-FV-010` | visualization | dedicated showcase profile/window keeps continuity-relevant satellites readable in the central high-elevation region without fake steering or HO truth | 6 |
...
| `VAL-PLAT-010` | exposure contract | `getProfileList()` importable from `@/core/contracts/exposure-v1`; returns Array of length 16; all 16 active profile IDs present (see `phase4-runtime-contract-sdd.md §8.3` for exact ID list); every entry has `tier ∈ ['Realistic','Advanced','Sensitivity']`; every entry has non-empty `id`, `label`, `family`; data NOT sourced from hardcoded `PROFILE_OPTIONS` | P4 | `scripts/validate-contracts.mjs` (runtime execution under `node --import tsx`). Exact check in `phase4-runtime-contract-sdd.md §8.3` |
...
| `VAL-MODQN-004` | downstream MODQN parity | current-anchor parity bundle exists over shipped MODQN result truth, machine-verifies per-target comparison mode / parity label / deviation completeness, and preserves the disclosed proxy ceiling without exceeding PM1 claim limits | TP1 |
| `VAL-MODQN-BUNDLE-001` | downstream MODQN bundle adapter | typed consumer loader accepts only the frozen Phase 03A schema/timeline surface, rejects missing replay-critical fields, and preserves slot-grouped replay truth without recomputing geometry or handover decisions | Phase 03A Slice B |
| `VAL-MODQN-BUNDLE-002` | downstream MODQN bundle UI | bundle replay mode changes the active truth source, replay-incomplete bundles are rejected before presentation, slot controls advance the exported timeline, shared beam/link renderers consume exported slot truth, native overlay state restores after bundle-mode exit, assumptions/provenance/training disclosure remains distinct from native defaults, browser-side `external-directory` loads succeed only after adapter + replay-readiness validation, failed external loads do not poison the current valid truth, and reset-to-sample revokes external figure object URLs | Phase 03A Slice C/D + Slice 2 |
| `VAL-MODQN-BUNDLE-003` | downstream MODQN story dashboard | bundle mode preserves the first-screen truth obligations while promoting a richer story dashboard, bundle-backed charts render only from existing replay/projector data, the metadata/disclosure panel remains distinct, and both sample and external bundle sources reach the same dashboard path | Slice 3 |
| `VAL-MODQN-BUNDLE-004` | downstream MODQN replay truth hardening | bundle dashboard/HUD/scene truth surfaces remain aligned on serving-satellite, slot, and handover state, shared beam/link presentation stays replay-truth-driven through slot progression, and a non-trivial external bundle variant proves distinct accepted truth through the same scene/dashboard path without falling back to sample/native assumptions | Slice 4 |
| `VAL-MODQN-BUNDLE-005` | downstream MODQN producer diagnostics consumer surface | producer-owned policy diagnostics / explainability over additive bundle fields; proves diagnostics come from exported producer truth, older bundles disclose absence without invention, manifest metadata/disclosure stays separate from row-level explainability truth, multi-user slots stay anchored to the first exported replay row, and Slice 2/3/4 behavior remains intact | Slice 5 landed |
...
| VAL-BEAM-001 | V | `validate-visual-browser.ts` — HOBS live browser probe verifies fixed footprint radius, no UE-origin re-pinning for bounded-steering profiles, off-center beam displacement, and satellite-relative tracked-lattice earth-moving projection |
...
| VAL-EE-003 | structural | validate-ee-provenance.ts — canonical formula/spec/provenance/runtime/profile wording alignment for EP1 semantics and source roles |
...
| VAL-MODQN-003 | E + structural | validate-modqn-m3.ts (real runner result-surface stabilization, disclosure completeness, serialization, and `ModqnViewModel` consumption) |
| VAL-MODQN-004 | E + structural | validate-modqn-parity.ts (current-anchor parity bundle, per-target completeness, packaging-only export projection, and disclosed proxy ceiling) |
| VAL-MODQN-BUNDLE-001 | structural + engine | validate-modqn-bundle-adapter.ts (schema/version guard, timeline parser, fixture load, replay-frame grouping over the producer bundle contract, and landed `001G` legacy decision-mask compatibility fallback coverage for older valid bundles) |
| VAL-MODQN-BUNDLE-002 | V | validate-modqn-bundle-ui.ts (preflight replay-presentation guard plus browser automation for truth-source switching, slot stepping, exported slot truth in validation-probe/shared renderers, native overlay restoration, assumptions/provenance/training disclosure, valid external-directory load, replay-incomplete rejection, invalid-load truth preservation, reset-to-sample, and blob URL cleanup) |
| VAL-MODQN-BUNDLE-003 | V | validate-modqn-bundle-ui.ts (browser automation for first-screen story-dashboard obligations, bundle-backed KPI/chart rendering from existing projector data, disclosure separation, and sample + external dashboard-path parity) |
| VAL-MODQN-BUNDLE-004 | V | validate-modqn-bundle-ui.ts — browser automation proves dashboard/HUD/probe serving-slot-handover alignment, authoritative replay wording, shared beam/link replay-truth continuity, and distinct accepted truth from a non-trivial external bundle over the same dashboard path |
| VAL-MODQN-BUNDLE-005 | V | validate-modqn-bundle-ui.ts — browser automation proves producer-owned policy diagnostics render only from exported fields, older bundles disclose absence honestly, `optionalPolicyDiagnostics` remains metadata/disclosure-only, sample and external bundles both surface explainability when present, and multi-user slots stay anchored to the first exported replay row rather than a frontend-sorted user |
...
| VAL-RT-003 | E + structural | validate-real-trace-truth-path.ts — dedicated real-trace truth-path parity validator over validation-sized cache sample ticks |
| VAL-RT-004 | structural | validate-real-trace-wording.mjs — dedicated wording/provenance/validator consistency check for shipped real-trace runtime truth |
...
| VAL-FV-004 | V | `validate-visual-browser.ts` — deterministic BH proof exposes `energyBlocked` cells in browser automation and verifies `EarthFixedCellLayer` consumes the shared presentation-frame beam picks |
| VAL-FV-005 | V | `validate-visual-browser.ts` — HOBS live + DAPS showcase browser probes check time advance, visible-set membership, shared presentation-frame beam selection, and continuity-focused scatter suppression / non-event beam-sat limiting |
| VAL-FV-006 | V | `validate-visual-browser.ts` — browser probe verifies `BeamInfoOverlay` SINR/serving-sat truth, overlay-label/display-satellite consistency, and presentation-frame beam picks against raw snapshot beam truth published in the validation probe |
...
| VAL-FV-010 | V | `validate-visual-browser.ts` — `case9-daps-showcase` live probe verifies continuity-focus within the first `300 s` of sim time, central high-elevation serving truth, a readable local serving-beam neighborhood, and serving-centered fallback when no nearby supporting context satellite exists in truth |
...
| VAL-PLAT-010 | structural | validate-contracts.mjs — getProfileList() returns 17 profiles, no hardcoded data **PASS** (2026-04-15) |
...
| VAL-MODQN-003 | structural + engine | validate-modqn-m3.ts — real runner result surface, disclosure-complete metadata, serialization, and `ModqnViewModel` consumption **PASS** (2026-04-01) |
| VAL-MODQN-004 | structural + engine | validate-modqn-parity.ts — current-anchor parity bundle, per-target comparison-mode/label/deviation completeness, packaging-only export projection, and disclosed proxy-ceiling preservation **PASS** (2026-04-02) |
...
### Recently Landed Follow-on Gates

| VAL ID | Phase | Status | Note |
|---|---|---|---|
| VAL-MODQN-BUNDLE-003 | Slice 3 | ✅ PASS | Landed through the expanded `validate-modqn-bundle-ui.ts`; verifies the richer bundle story dashboard, bundle-backed KPI/chart rendering, disclosure separation, and sample/external dashboard-path parity |
| VAL-MODQN-BUNDLE-004 | Slice 4 | ✅ PASS | Landed through the expanded `validate-modqn-bundle-ui.ts`; verifies dashboard/HUD/probe replay-truth alignment, authoritative handover wording/counts, shared beam/link replay-truth proof, and non-trivial external-bundle acceptance |
| VAL-MODQN-BUNDLE-005 | Slice 5 | ✅ PASS | Landed through the expanded `validate-modqn-bundle-ui.ts`; verifies producer-owned diagnostics rendering, older-bundle absence disclosure, metadata/disclosure separation, sample/external explainability parity, and multi-user row anchoring without regressing Slice 2/3/4 |

**Note:** Formula-level (`-F`) checks are automated and pass. Engine-level (`-E`) checks pass and are part of `npm run validate:stage` via `node --import tsx`. Engine coverage now extends through E-11, including `VAL-POLICY-001` and `VAL-DOPPLER-001` in addition to the earlier E-5 through E-9 expansion. `npm run validate:integration` runs all engine + reproduction checks. Browser-level visual checks (`VAL-BEAM-001`, `VAL-FV-004` through `VAL-FV-010`, `VAL-EXP-001`, and `VAL-MODQN-BUNDLE-002` through `VAL-MODQN-BUNDLE-005`) are automated via `validate-visual-browser.ts` / `validate-modqn-bundle-ui.ts`, and `VAL-ORB-001` is now browser/headless-hybrid validated by `validate-orbit-parity.ts`. Platform Refactor contract gates VAL-PLAT-008/009/010 are enforced via `validate-contracts.mjs`, Phase 5 structural gates VAL-PLAT-011/012 are enforced via `validate-structure.mjs`, EP1 `EE / power` hardening gates VAL-EE-003/004 are enforced via `validate-ee-provenance.ts` and `validate-ee-disclosure.ts`, and the Phase 03A bundle-consumer gates VAL-MODQN-BUNDLE-001/002/003/004/005 are enforced via `validate-modqn-bundle-adapter.ts` and `validate-modqn-bundle-ui.ts`. The latest targeted reruns passed, but the browser-visible subset should still be treated as transient-sensitive.
...
|---|---|---|---|---|
| `VAL-PLAT-001` | P1 | ✅ PASS | 2026-04-13 | All 60 canonical parameterPaths present in PARAMETER_REGISTRY; all profile-specific binding defaults match runtime profiles |
| `VAL-PLAT-002` | P1 | ✅ PASS | 2026-04-11 | All 38 distinct sourceIds resolve in paper-sources.json (nested-section lookup) |
| `VAL-PLAT-003` | P1 | ✅ PASS | 2026-04-13 | 60 PARAM-* IDs: unique, prefixed, no collision with source namespace |
...
| `VAL-PLAT-005` | P2 | ✅ PASS | 2026-04-15 | `buildModelBundle` produced valid non-null bundles for all 16 active profiles (per SDD §8.2 validation requirement). Re-verified after the showcase-profile registration |
...
| `VAL-PLAT-007` | P3 | ✅ PASS | 2026-04-15 | `scripts/validate-profiles.mjs` — authored registry parity confirmed for all 16 active profiles via `materializeRuntimeProfile(entry.bundle, entry.exp)` deep-equality against `DEFAULT_PROFILES[entry.id]` |
...
| `VAL-MODQN-003` | M3 | ✅ PASS | 2026-04-01 | `validate-modqn-m3.ts` runs the real reproduction path, verifies additive result-surface stabilization, disclosure-complete metadata, serialization, and truth-safe `ModqnViewModel` projection without breaking `VAL-MODQN-002` |
| `VAL-RT-003` | T1 | ✅ PASS | 2026-04-01 | `validate-real-trace-truth-path.ts` verifies validation-sized real-trace cache samples match `propagateSgp4()` / satellite.js at sampled ticks without changing replay identity semantics |
| `VAL-RT-004` | T1 | ✅ PASS | 2026-04-01 | `validate-real-trace-wording.mjs` verifies defaults, registry notes, UI exposure wording, implementation status, and validator assumptions all describe the shipped OMM/TLE ingest + SGP4-sampled cache truth consistently |
| `VAL-EE-003` | EP1 | ✅ PASS | 2026-04-02 | `validate-ee-provenance.ts` verifies formulas/spec/provenance/runtime/profile wording all keep the EP1 semantic split and source-role discipline aligned |
| `VAL-EE-004` | EP1 | ✅ PASS | 2026-04-02 | `validate-ee-disclosure.ts` verifies active-TX EE vs broader total-power proxy runtime semantics, artifact disclosure surfaces, and the minimum sensitivity-qualified claim bar for denominator-sensitive EE |
