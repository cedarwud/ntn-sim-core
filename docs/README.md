...
The simulator-platform refactor is complete, the shipped downstream baseline lines are closed through `MODQN M3` / `UI U2`, the narrow real-trace truth-path correction (`T1`) is shipped, and the current active follow-on is `PM1` paper-mode / claim-mode hardening.
...
14. `../todo/README.md`
   - current handoff index across shipped downstream baselines, the active `PM1` follow-on, and deferred future tracks
...
4. shipped downstream baseline work (`M1` -> `M3`, `U1` -> `U2`, plus the separately shipped `T1` truth-path correction) should be read from the active shipped downstream surface defined in `../sdd/README.md`, `downstream-runtime-architecture-sdd.md`, and the matching baseline specs; the current active next step is `PM1` paper-mode / claim-mode hardening, while any reopened `MODQN` / `UI` / `estnet` / runtime work still requires fresh promotion or explicit reopen.
