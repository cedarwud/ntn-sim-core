...
- 下一個合法主題是 downstream baseline entry：已可正式進入 `MODQN M1` 或 `UI U1`
- `project/estnet-ui-kickoff` 仍維持 paused future consumer 狀態

因此閱讀本專案時，應把它視為 **已完成 platform closure 並已建立 downstream baseline entry surface 的研究型模擬器主體**。接下來的主要工作不是再開新的 platform-refactor group，而是沿著已啟用的 downstream active surface 啟動 `M1` / `U1`；更後段的 `M2` / `M3` 與 `estnet` 仍需各自的 promotion / reopen gate。
...
NTN Sim Core is a research-oriented NTN/LEO simulator platform. It keeps the NTPU campus scene plus UAV and satellite assets, and the simulator-platform refactor into the `app / viz / core / runner` layered architecture is already complete. Downstream architecture Group 2 has also landed, so `MODQN M1` and `UI U1` now enter through active baseline surfaces rather than outline-only placeholders.
