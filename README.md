...
| 目前 active paper-oriented follow-on | [sdd/paper-mode-claim-mode-hardening-outline.md](./sdd/paper-mode-claim-mode-hardening-outline.md) / [../todo/paper-mode-claim-mode/README.md](../todo/paper-mode-claim-mode/README.md) |
| 目前 prompt / handoff 索引 | [../todo/README.md](../todo/README.md) / [../todo/paper-mode-claim-mode/README.md](../todo/paper-mode-claim-mode/README.md) / [../todo/modqn/README.md](../todo/modqn/README.md) / [../todo/ui-estnet/README.md](../todo/ui-estnet/README.md) / [../todo/real-trace-truth-path/README.md](../todo/real-trace-truth-path/README.md) |
...
- paper-oriented follow-on 已 promotion 成 `paper-mode / claim-mode hardening`；目前應先從這條治理/packaging surface 收斂論文使用規則
- `project/estnet-ui-kickoff` 仍維持 paused future consumer 狀態，需 explicit reopen 才能啟動
- 目前若要為論文導向的後續開發定 scope，應先參考 [`sdd/ntn-sim-core-research-positioning-note.md`](./sdd/ntn-sim-core-research-positioning-note.md)；`OMNeT++ / estnet / INET` 若要重開，應視為新的外部 consumer/backend realism track，而不是延續 `U1/U2/T1`

因此閱讀本專案時，應把它視為 **已完成 platform closure，且已收口目前批准的 downstream baseline 與 narrow truth-path correction 的研究型模擬器主體**。目前若要繼續往論文導向工作前進，應先走 `paper-mode / claim-mode hardening`，而不是回到已完成的 `M1/U1/T1` 入口，也不是直接跳到更重的 `OMNeT++ / estnet / INET` 或 scalability line。
