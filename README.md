...
- single-repo dual-app showcase 的 current-tree runtime baseline 仍然是 landed：`AppShell` 保留 `?app=showcase-consumer` 路徑，`showcase-consumer.html` + `src/showcase-consumer-main.tsx` packaged viewer 仍存在；`ShowcaseConsumerHost` 仍是唯一 publisher，依 `showcasePath=native-replay|bundle-sample` 發布 allowlisted starter surface，而 `ShowcaseConsumerApp` 仍只消費 `sceneConsumerStarter`。固定基線 `scene-consumer-starter-v1` / `scene-consumer-starter-v2`、deterministic path、`summary.*` secondary、`Primary SINR = snapshot.ues[0].sinrDb` 都維持不變；`live` / `external-directory` / bundle panel migration 仍然關閉
- dual-app 現在新增一個 landed authority-chain repair record `single-repo-dual-app-showcase-authority-chain-repair-follow-on.md`，用來修復 active docs 對不存在 dual-app phase SDD 的引用；同時也新增一個 promoted planning-only follow-on `single-repo-dual-app-showcase-phase3-rendering-follow-on.md`，把 dual viewer 正式重開到「scene-first rendering / presentation finish line」這一層，但仍不重開 truth/source semantics、route retirement、redirect work、`live`、`external-directory`、或非 additive 的 ownership 變更
...
最新 dual-app active authority 現在改由 `single-repo-dual-app-showcase-authority-chain-repair-follow-on.md` + `single-repo-dual-app-showcase-mainline-additive-reintegration-follow-on.md` 描述 current-tree landed baseline，並由 `single-repo-dual-app-showcase-phase3-rendering-follow-on.md` 作為新的 promoted planning-only reopen surface。也就是說，dual-app runtime baseline 沒變，但 Phase 3 rendering/presentation finish line 現在已重新進入正式規劃。
