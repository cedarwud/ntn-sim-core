...
- 目前沒有新 promotion 的 paper-oriented active queue；若要再往下做，應重新 promotion 新的 follow-on，而不是直接把 `TP1` 當成未關閉的 active line

因此閱讀本專案時，應把它視為 **已完成 platform closure，且已收口目前批准的 downstream baseline、narrow truth-path correction、與 current-anchor parity package 的研究型模擬器主體**。目前若要繼續往論文導向工作前進，應先重新 promotion 新的 follow-on，而不是回到已完成的 `M1/U1/T1/PM1/TP1` 入口，也不是直接跳到更重的 `OMNeT++ / estnet / INET` 或 scalability line。
