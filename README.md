...
| 目前 downstream prompt 入口 | [../todo/modqn/README.md](../todo/modqn/README.md) / [../todo/ui-estnet/README.md](../todo/ui-estnet/README.md) |
...
- 下一個合法主題是 downstream preflight / promotion：`MODQN M1` 或 `UI U1`
- `project/estnet-ui-kickoff` 仍維持 paused future consumer 狀態

因此閱讀本專案時，應把它視為 **已完成 platform closure 的研究型模擬器主體**。接下來的主要工作不是再開新的 platform-refactor group，而是把 downstream outline（MODQN / UI）重寫或升格成 active spec surface 後再實作。
...
- `src/core/`
  simulator truth layer；含 `config/`, `contracts/`, `models/`, `profiles/`, `engine/`, `orbit/`, `channel/`, `handover/`, `energy/`, `beam/`, `trace/`, `policy/`, `kpi/`
- `src/runner/`
  `headless/`, `replay/`, `curation/`，以及 `runner-exposure-api.ts`
- `src/viz/`
  `beam/`, `satellite/`, `scene/`, `overlays/`, `validation/`
- `src/assets/`
...
NTN Sim Core is a research-oriented NTN/LEO simulator platform. It keeps the NTPU campus scene plus UAV and satellite assets, and the simulator-platform refactor into the `app / viz / core / runner` layered architecture is already complete. MODQN and new UI work now enter as downstream programs.
...
The authoritative inventory lives in [Implementation Status §4](./sdd/ntn-sim-core-implementation-status.md). The older preflight skeleton is no longer representative of the current tree.
...
- `src/core/`
  simulator truth layer; includes `config/`, `contracts/`, `models/`, `profiles/`, `engine/`, `orbit/`, `channel/`, `handover/`, `energy/`, `beam/`, `trace/`, `policy/`, and `kpi/`
- `src/runner/`
  `headless/`, `replay/`, `curation/`, plus `runner-exposure-api.ts`
- `src/viz/`
  `beam/`, `satellite/`, `scene/`, `overlays/`, and `validation/`
- `src/assets/`
  scene / model asset path registry
- `src/styles/`
  global Sass entry (`main.scss`)
